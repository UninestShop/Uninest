                ...
                <div class="mypro-content">
                    <div class="top-head">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            @if($unreadChatCount > 0)
                                <span class="badge bg-danger ms-2 text-white">{{ $unreadChatCount }} Unread</span>
                            @endif
                        </div>
                        <div class="btn-group" role="group">
                            <button type="button" class="btn btn-outline-primary filter-btn active" data-filter="all">All Messages</button>
                            <button type="button" class="btn btn-outline-success filter-btn" data-filter="selling">My Listings</button>
                            {{-- <button type="button" class="btn btn-outline-info filter-btn" data-filter="buying">Products I'm Buying</button> --}}
                        </div>
                    </div>
                    <div id="chats-container">
                        @forelse($chats as $productId => $productChats)
                            @php 
                                $firstChat = $productChats->first(); 
                                $product = $firstChat->product;
                                if (!$product) continue; // Skip if product doesn't exist anymore
                                
                                $hasUnread = $productChats->contains(function($chat) {
                                    return ($chat->receiver_id === auth()->id() && !$chat->is_read) || 
                                        ($chat->product && $chat->product->user_id === auth()->id() && $chat->has_unread_seller_messages);
                                });
                                
                                $isSeller = $product->user_id === auth()->id();
                                $chatRole = $isSeller ? 'selling' : 'buying';
                            @endphp
                            
                            <div class="card mb-3 {{ $hasUnread ? 'border-primary' : '' }} chat-card" data-role="{{ $chatRole }}">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-2">
                                            @if($product->photos)
                                        ...
                                        <div class="col-md-7">
                                            <div class="d-flex align-items-center mb-1">
                                                ...
                                                <span class="badge {{ $isSeller ? 'bg-success' : 'bg-info' }}">
                                                    {{ $isSeller ? 'You\'re Selling' : 'You\'re Buying' }}
                                                </span>
                                                @if($hasUnread)
                                                    <span class="rounded-pill bg-danger ms-auto text-white pulse-animation px-2 py-1" style="font-size: 0.7rem;">
                                                        <i class="fas fa-bell me-1"></i>
                                                         {{ $productChats->where(function($chat) {
                                                            return ($chat->receiver_id === auth()->id() && !$chat->is_read) || 
                                                                ($chat->product && $chat->product->user_id === auth()->id() && $chat->has_unread_seller_messages);
                                                        })->count() }} 
                                                        Unread
                                                    </span>
                                                    {{-- <span class="badge bg-danger ms-auto text-white">
                                                        {{ $productChats->where(function($chat) {
                                                            return ($chat->receiver_id === auth()->id() && !$chat->is_read) || 
                                                                ($chat->product && $chat->product->user_id === auth()->id() && $chat->has_unread_seller_messages);
                                                        })->count() }} 
                                            ...
                                            <div class="d-flex flex-column">
                                                @php
                                                    // Group chats by users for this product
                                                    $chatsByUser = $productChats->groupBy(function($chat) {
                                                        return auth()->id() === $chat->sender_id ? $chat->receiver_id : $chat->sender_id;
                                                    });
                                                @endphp
                                                
                                                @foreach($chatsByUser as $userId => $userChats)
                                                    @php
                                                        $otherUser = $userChats->first()->sender_id === auth()->id() 
                                                            ? $userChats->first()->receiver 
                                                            : $userChats->first()->sender;
                                                            
                                                        if (!$otherUser) continue; // Skip if user doesn't exist
                                                        
                                                        $lastChat = $userChats->sortByDesc('created_at')->first();
                                                        $hasUnreadFromUser = $userChats->contains(function($chat) {
                                                            return $chat->receiver_id === auth()->id() && !$chat->is_read;
                                                        });
                                                    @endphp
                                                    ...
                                                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                                                        <div>
                                                            <p class="mb-0 {{ $hasUnreadFromUser ? 'fw-bold' : '' }}">
                                                                {{ $otherUser->name }}
                                                            ...
                                                            <small class="text-muted">
                                                                {{ Str::limit($lastChat->message, 40) }} · {{ $lastChat->created_at->diffForHumans() }}
                                                            </small>
                                                        </div>
                                                        <a href="{{ route('messages.show', $lastChat) }}" class="btn btn-sm btn-outline-primary">
                                        ...
                                        <div class="col-md-3 d-flex flex-column justify-content-between align-items-end">
                                            <div>
                                                <p class="text-success mb-1">${{ number_format($product->selling_price, 2) }}</p>
                                                <span class="badge bg-secondary mb-2">{{ $product->condition }}</span>
                                            </div>
                                            <div class="mt-2">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        filterButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                const visibleCards = Array.from(chatCards).filter(card => card.style.display !== 'none').length;
                ...
                if (visibleCards === 0) {
                    if (!emptyState) {
                        ...
                        emptyDiv.innerHTML = `
                            ...
                            <p class="mb-0">No ${filterValue === 'selling' ? 'selling' : 'buying'} conversations found</p>
                        `;
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <a href="{{ route('messages.index') }}" class="btn btn-primary">Cancel</a>

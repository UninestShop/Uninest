@extends('layouts.app')
@php
    // Calculate unread chat count once at the top of the file
    $unreadChatCount = 0;
    if (isset($chats)) {
    ...
    <div class="container">
            ...
            <div class="profile-left">
                <div class="profile-top">
                    ...
                    <ul class="profile-list">
                        ...
                        {{-- <li><a href="{{ route('messages.index','#profileRight') }}" class="pro-links {{ request()->routeIs('messages.*') ? 'active' : '' }}"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            </svg> My Messages</a></li> --}}
                         <li>
                                <a href="{{ route('messages.index','#profileRight') }}" class="pro-links pro-linkss d-flex align-items-center {{ request()->routeIs('messages.*') ? 'active' : '' }}">
        ...
        @endif
                <div class="mypro-content">
                    ...
                    @endif
                    <div class="top-head" id="editmyproducts">
                        <h4>Edit a Product</h4>
                    </div>
                    <div class="">
                        <form class="login-form profile-form" action="{{ route('seller.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="mrp" class="form-control" value="{{ old('mrp', $product->mrp) }}" min="0">
                                    </div>
                                </div>
                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Category <span style="color:red">*</span></label>
                                    <select name="category_id" required id="categorySelect" class="form-control">
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="location" id="inputAddressvalue" placeholder="Type to search for a location" value="{{ old('location', $product->location) }}" required>
                            ...
                            <div class="mb-3">
                              ...
                              @php
                                // Handle different possible formats of payment_method (string, array or null)
                                $paymentMethods = [];
                                if (isset($product->payment_method)) {
                                  if (is_string($product->payment_method)) {
                                    $paymentMethods = json_decode($product->payment_method, true) ?: [];
                                  } else {
                                    $paymentMethods = is_array($product->payment_method) ? $product->payment_method : [];
                                  }
                                }
                                // Merge with old input if available
                                $paymentMethods = old('payment_method', $paymentMethods);
                              @endphp
                              <select name="payment_method[]" class="form-control select2-multiple" multiple required>
                                <option value="Cash" {{ in_array('Cash', $paymentMethods) ? 'selected' : '' }}>Cash</option>
                                <option value="Venmo" {{ in_array('Venmo', $paymentMethods) ? 'selected' : '' }}>Venmo</option>
                                <option value="Paypal" {{ in_array('Paypal', $paymentMethods) ? 'selected' : '' }}>Paypal</option>
                                <option value="Zelle" {{ in_array('Zelle', $paymentMethods) ? 'selected' : '' }}>Zelle</option>
                                <option value="Free" {{ in_array('Free', $paymentMethods) ? 'selected' : '' }}>Free</option>
                              </select>
                              @error('name')
                              <div class="invalid-feedback">
                                  <strong>{{ $message }}</strong>
                          ...
                          @enderror
                            ...
                            <div class="mb-3">
                                <label class="form-label">Upload Images (Max 5) <span style="color:red">*</span></label>
                                <div class="d-flex align-items-center mb-2">
                                    ...
                                    <div class="ms-2 text-muted small">
                                        <span id="imageCounter">
                                            @php
                                                $existingImageCount = is_array($product->photos) ? count($product->photos) : 0;
                                                echo $existingImageCount;
                                            @endphp
                                        </span>/5 images selected (including <span id="existingImageCount">{{ $existingImageCount }}</span> current images)
                                    </div>
                                </div>
                            ...
                            <div class="mb-3">
                              ...
                              <div class="row">
                                          ...
                                          <div class="col-md-3 mb-2">
                                              <div class="card">
                                                  ...
                                                  <div class="card-body p-2">
                                                      <button type="button" class="btn btn-sm btn-danger w-100 remove-image-btn">Remove</button>
                                                      <input type="hidden" name="removed_photos[]" value="{{ $photo }}" disabled>
                                                  </div>
                                              </div>
                                          </div>
                                      @endforeach
                                  @else
                                      <div class="col-12">
                                          <div class="alert alert-info mb-0">No images currently uploaded.</div>
                                      </div>
                                  @endif
                              </div>
                          </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize with existing images count
        let existingImages = document.querySelectorAll('.remove-image-btn').length;
        let removedExistingImages = 0;
        let newlyAddedImages = 0;
        
        // Function to update image counter and check limits
        function updateImageCounter() {
            const totalActiveImages = existingImages - removedExistingImages + newlyAddedImages;
            imageCounter.textContent = totalActiveImages;
            
            // Update existing image count display
            const existingImageCount = document.getElementById('existingImageCount');
            if (existingImageCount) {
                existingImageCount.textContent = existingImages - removedExistingImages;
            }

            // Enable/disable remove buttons based on active images
            const allRemoveButtons = document.querySelectorAll('.remove-image-btn');
            const activeImages = Array.from(allRemoveButtons).filter(btn => 
                !btn.closest('.col-md-3').style.display || 
                btn.closest('.col-md-3').style.display !== 'none'
            );

            allRemoveButtons.forEach(btn => {
                const isLastActive = activeImages.length === 1 && activeImages[0] === btn;
                btn.disabled = isLastActive;
                btn.title = isLastActive ? 'Cannot remove last image' : '';
            });
            
            return totalActiveImages;
        }
        ...
        addImageBtn.addEventListener('click', function() {
            const totalActiveImages = updateImageCounter();
            if (totalActiveImages < maxImages) {
                imageInput.click();
            } else {
            ...
            if (this.files && this.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    removeBtn.addEventListener('click', function() {
                        document.getElementById('preview_' + this.dataset.id).remove();
                        document.getElementById('file_' + this.dataset.id).remove();
                        newlyAddedImages--;
                        updateImageCounter();
                    });
        ...
        // Replace the existing image removal event listener with this simplified version
        document.querySelectorAll('.remove-image-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                // Check if this would be the last remaining image
                if (activeImages.length === 1 && activeImages[0] === this) {
                    // Show error message for last image
                    const errorMessage = document.createElement('div');
                    errorMessage.className = 'alert alert-danger mt-2';
                    errorMessage.textContent = 'Cannot remove the last image. At least one image is required.';
                ...
                const imageCol = this.closest('.col-md-3');
                
                // Simply hide the image and mark for removal
        ...
        // Modify the code for the newly added images' remove buttons
        const originalHandleFileSelection = imageInput.onchange;
        ...
        // Add form submit event listener to verify removal status
        document.getElementById('productForm').addEventListener('submit', function() {
            // Check if any images are marked for removal
            const removedImages = document.querySelectorAll('input[name="removed_photos[]"]:not([disabled])');
            console.log(`${removedImages.length} images marked for removal`);
            
            // Check total active images
            const totalActiveImages = existingImages - removedExistingImages + newlyAddedImages;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('logoutBtn').addEventListener('click', function(e) {
            ...
            // Try using Bootstrap's modal API if available
